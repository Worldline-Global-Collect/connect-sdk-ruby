...
module Integration
  ...
  def self.init_communicator_configuration(properties_url: PROPERTIES_URI, max_connections: nil)
    ...
    configuration = Worldline::Connect::SDK::CommunicatorConfiguration.new(properties: yaml,
                                                                           api_endpoint: 'https://api.preprod.connect.worldline-solutions.com',
  ...
  def self.init_client(with_proxy, max_connections = nil)
    ...
    yield client = Worldline::Connect::SDK::Factory.create_client_from_configuration(configuration).with_client_meta_info('{"test":"test"}')
    client.close
  end
end
