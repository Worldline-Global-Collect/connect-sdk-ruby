...
def run_request(i, communicator, flag)
  ...
  client = Worldline::Connect::SDK::Factory.create_client_from_communicator(communicator)
  flag.wait
  start_time = Time.now
  client.v1.merchant(Integration::MERCHANT_ID).services.convert_amount(request)
