...
Factory ||= Worldline::Connect::SDK::Factory

describe 'Factory' do
  it 'can initialize configurations' do
    configuration = Factory.create_configuration(PROPERTIES_URI, API_KEY_ID, SECRET_API_KEY)

    expect(configuration.api_endpoint).to eq('https://api.preprod.connect.worldline-solutions.com')
  ...
  it 'can initialize communicators' do
    ...
    expect(communicator.marshaller).to be(Worldline::Connect::SDK::JSON::DefaultMarshaller.instance)
    expect(connection).to be_an_instance_of(Worldline::Connect::SDK::Communication::DefaultConnection)
    expect(authenticator).to be_an_instance_of(Worldline::Connect::SDK::Authentication::V1HMACAuthenticator)
    expect(metadata_provider).to be_an_instance_of(Worldline::Connect::SDK::Communication::MetadataProvider)
